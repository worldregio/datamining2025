...
Télécharger le [dossier projet exos2](https://github.com/worldregio/datamining2025/raw/main/exos/exo2.zip) pour suivre et executer les commandes.
...
download.file(url="https://www.insee.fr/fr/statistiques/fichier/6544333/RP2019_INDCVIZA_csv.zip",
              destfile = "ParisPC/tmp/RP2019_INDCVIZA_csv.zip")
unzip("ParisPC/tmp/RP2019_INDCVIZA_csv.zip", exdir = "ParisPC/tmp")

## Lecture du fichier individu avec fread
library(data.table)
RP <- fread("ParisPC/tmp/FD_INDCVIZA_2019.csv", stringsAsFactors=TRUE)
RP <- as.data.frame(RP)
## Selection Paris PC 
RP <- RP %>% filter(DEPT %in% c(75, 92, 93, 94))
saveRDS(RP, "ParisPC/data/indiv2019.RDS")

## Lecture du fichier de métadonnées
meta <- read.csv(file = 'ParisPC/tmp/Varmod_INDCVI_2019.csv',
                 sep = ";",
                 encoding = "UTF-8",
                 stringsAsFactors = TRUE)

## Sauvegarde des deux fichiers
saveRDS(meta, "ParisPC/data/meta.RDS")
...
indiv <- readRDS("ParisPC/data/indiv2019.RDS")
