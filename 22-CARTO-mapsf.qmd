---
title: "Cartographie statique avec mapsf"
...
map_iris<-readRDS("parisPC/geom/map_iris_csp.RDS") %>% st_transform(crs=2154)
map_com <-readRDS("parisPC/geom/map_com_csp.RDS") %>% st_transform(crs=2154)
map_dep <-readRDS("parisPC/geom/map_dep.RDS") %>% st_transform(crs=2154)
```
...
# Ajoute un cadre, un titre et des sources
mf_layout(title = "Communes et Iris de Paris + PC en 2021", 
          frame = TRUE,
          credits = "Sources : IGN ")
```
...
```{r }
# Calcule le nombre total d'actif
map_iris$ACT <- map_iris$CHO+map_iris$EMP
...
mf_map(map_iris, var = "ACT",type = "prop",
  inches = 0.1, col = "red",leg_pos = "left",  
  leg_title = "Nombre d'actifs", add=TRUE)

mf_layout(title = "Distribution des actifs en 2021", 
          frame = TRUE,
          credits = "Sources : IGN et INSEE RP 2021")
...
map_iris$TxCHO<-100*map_iris$CHO/map_iris$ACT
```
...
mybreaks = c(0, 4, 6, 8, 10, 12, 14, 16, 18,20,50)

# Tracer la carte choroplèthe
mf_map( map_iris, var = "TxCHO",type = "choro",
...
mf_layout(title = "Taux de chômage en 2021", 
          frame = TRUE,
          credits = "Sources : IGN et INSEE RP 2021")
```
...
mf_prop_choro( x = map_iris,  var = c("ACT", "TxCHO"), 
  ...
  add = TRUE)
mf_layout(title = "Les actifs au chômage en 2021",
        frame = TRUE, credits = "Sources : IGN et INSEE RP 2021")
```   
...
levels(map_com_typo$typoqual) <- c("(1) Etudiants ++",
                                   "(2) Retraités ++ Actifs +",
                                   "(3) Actifs ++",
                                   "(4) Chômeurs + Autres +",
                                   "(4) Autres ++ Chômeurs ++")
...
mf_map(map_com_typo, 
       type = "typo",
       var = "typoqual",
       pal= c("lightblue","lightgreen","lightyellow","pink","red"),
...
mf_layout(title = "Typologie des profls activités des communes de Paris et Petite Couronne en 2021",
        frame = TRUE, credits = "Sources : IGN et INSEE RP 2021",
        arrow = F
        )
